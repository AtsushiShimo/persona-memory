...
def set_debug_mode(
    ...
    背景: 0.7.7 までは PERSONA_MEMORY_DEBUG 環境変数だとプロセス起動時にしか
    切替できず、 セッション維持したまま 「不具合を調査するため一時的に DB を
    覗きたい」 場面に対応できなかった. 0.7.8 で本ツール (= flag 経路) を追加.
    0.8.8 以降は旧 env 経路 (PERSONA_MEMORY_DEBUG) を撤去し flag 一本化.
