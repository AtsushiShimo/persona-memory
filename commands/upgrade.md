...
env = data.get("env") or {}
target = f"{prefix}: "
if env.get("CLAUDE_REMOTE_CONTROL_SESSION_NAME_PREFIX") == target:
    print(f"[ok] env CLAUDE_REMOTE_CONTROL_SESSION_NAME_PREFIX 既に最新 ({target})")
else:
    env["CLAUDE_REMOTE_CONTROL_SESSION_NAME_PREFIX"] = target
    data["env"] = env
    fp.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"[ok] {fp} に CLAUDE_REMOTE_CONTROL_SESSION_NAME_PREFIX={target} を設定")
PY
