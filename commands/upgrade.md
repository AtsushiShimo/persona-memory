...
# 2. .envrc に persona-memory ブロックを追加/更新 (direnv 経由で env 連携)
echo
echo "=== .envrc を更新 ==="
...
if ! command -v direnv >/dev/null 2>&1; then
  echo "[warn] direnv が未インストール。.envrc は更新しましたが auto-export は"
  echo "       されません。手動 export か direnv インストール推奨:"
  echo "         brew install direnv && eval \"\$(direnv hook zsh)\""
fi

# 3. (旧 0.4.7 の名残) settings.local.json から env を除去
...
if removed:
    print(f"[cleanup] 旧 settings.local.json から env を除去 (direnv 経路に移行)")
PY
fi
...
echo "(direnv 利用時) 'direnv allow' を実行してから claude を起動。"
```
