"""UserPromptSubmit hook 同期処理 (phase 2 範囲).

実行内容:
1. 機密チェック (検出時は raw 保存 / recall すべてスキップ、stderr 警告)
2. user 発話を episodes に raw 保存 (zero-loss)
3. recall は phase 4 で追加 (現状は何も注入しない)
4. write は phase 3 で追加 (現状は detach 起動なし)
...
def main() -> int:
    ...
    try:
        ...
        try:
            save_episode(conn, role="user", content=prompt, session_id=session_id)
        finally:
            conn.close()
