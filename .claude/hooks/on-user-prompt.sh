#!/bin/sh
# UserPromptSubmit hook — phase 2 (raw save + secret check).
# 旧版 (proxy_recall.py = recall) は phase 4 で復活させる。

# 子セッション (claude -p) では skip
...
PYTHONPATH="$SCRIPT_HOME${PYTHONPATH:+:$PYTHONPATH}" \
  "$PERSONA_PYTHON" -m scripts.hooks.on_user_prompt
