...
def llm_judge(
    ...
    prompt = (
        "以下のアシスタント応答に、期待される各事実が「意味的に正しく」含まれているか判定してください。\n"
        "- yes = 事実そのもの or 等価な意味で言及している (paraphrase OK)\n"
        "- no = 言及なし or 反対の意味で言っている (例: 期待が「Java を避けたい」なのに応答が「Java で書こう」)\n"
        "- unsure = 判定材料が足りない / 曖昧\n\n"
        f"応答:\n{excerpt}\n\n"
        f"期待される事実 (各行 1 件):\n{facts_block}\n\n"
